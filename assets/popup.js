(function () {
  ...
  const styles = `
    ...
    .modal{background:#fff;border-radius:16px;padding:2.2rem 2rem 2rem;max-width:460px;width:100%;box-shadow:0 24px 60px rgba(0,0,0,.3);animation:m-in .32s cubic-bezier(.34,1.4,.64,1);font-family:Arial,sans-serif;text-align:center;border-top:4px solid #0a4a6b;}
    @keyframes m-in{from{opacity:0;transform:scale(.91) translateY(14px)}to{opacity:1;transform:scale(1) translateY(0)}}
    .modal h3{font-size:1.15rem;font-weight:700;color:#0a4a6b;margin:0 0 .6rem;}
    .modal p{font-size:.87rem;color:#555;line-height:1.7;margin:0 0 1.5rem;}
    .modal p a{color:#0a4a6b;text-decoration:underline;}
    .modal-divider{border:none;border-top:1px solid #ebebeb;margin:0 0 1.5rem;}
    .modal-actions{display:flex;gap:.75rem;}
    .mbtn{flex:1;padding:.82rem 1rem;border-radius:8px;font-size:.9rem;font-weight:700;cursor:pointer;transition:background .2s,transform .15s;border:none;background:#0a4a6b;color:#fff;}
    .mbtn:hover{background:#062e44;transform:translateY(-1px);}
    .mbtn.ghost{background:#fff;color:#0a4a6b;border:2px solid #0a4a6b;}
    .mbtn.ghost:hover{background:#e0f2fe;transform:translateY(-1px);}
  ...
  function buildPopup() {
    ...
    bd.innerHTML = `
      <div class="modal" role="dialog" aria-modal="true">
        <h3>🦷 You are almost in!</h3>
        <p>Confirm you are <strong>18 or older</strong> and agree to our terms to access the full dental supplement review and current offers.</p>
        <hr class="modal-divider">
        <div class="modal-actions">
          <button class="mbtn"       id="p-yes">✅ Yes, I am 18+</button>
          <button class="mbtn ghost" id="p-no">✕ No</button>
        </div>
...
window.location.href = "privacy.html";
  ...
  // lander.html — always shows, both redirect
